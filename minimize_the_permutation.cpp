...
int main()
{
    ...
    testcases
    {
        ...
        vi arr(n);
        ai(arr,n);
        ll int pos=0;// jaha pe hme insert krna hai wo position
        while(pos<n)
        {
			int index = min_element(arr.begin()+pos,arr.end())-arr.begin();
			int element = arr[index];
			arr.erase(arr.begin()+index);
			arr.insert(arr.begin()+pos,element);
			if(pos==index)
			{
				pos=index+1; // yha par hmne is isliye kyoki hham index se pehle koi bhhi swapping nahi kar skte bacause e
				// at each index operation can be performed only once.
			}
			else
			{
				pos=index;
